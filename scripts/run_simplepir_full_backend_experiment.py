...
def build_pbc_active_indexes(
    ...
    """PBC baseline layout for the same SimplePIR backend.
...
def write_note(path: Path, rows: Sequence[Dict[str, object]], args: argparse.Namespace) -> None:
    ...
    lines.extend(
        [
            ...
            "- `pbc_active` is the PBC organization over the same active proof records: three replicated copies and about `1.5m` bucket queries, all executed by the same SimplePIR runner.",
