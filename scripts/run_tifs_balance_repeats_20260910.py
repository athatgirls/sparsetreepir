...
def summarize():
    ...
    for (label, algorithm), chosen in sorted(groups.items()):
        ...
        summary = dict(label=label, algorithm=algorithm, attempts=len(chosen), completed=len(complete),
                       ...
                       equal_quality_completed_runs=(len({tuple(sorted(r["loads"])) for r in complete}) <= 1) if complete else None,
...
def write_findings(summaries):
    ...
    table += [r"\begin{table}[H]", r"\centering", r"\caption{Three-process repetition at 100\,000 uniform occupied coordinates and height 128, with a 20-scan target and a 120-s process limit. Censored runs are not treated as completed timings.}", r"\label{tab:supp_balance_repeats_scale}", r"\footnotesize", r"\setlength{\tabcolsep}{4pt}", r"\begin{tabular}{lcccl}", r"\toprule", r"Method & Complete & Coloring s & Finished scans & Endpoint $U/B$ \\", r"\midrule",
              "Old / Hungarian & " + f"{old['completed']}/3" + r" & not completed & " + ",".join(map(str, old["completed_scan_rounds"])) + " & " + ",".join(f"{x:.4f}" for x in old["endpoint_U_over_B"]) + r" \\",
              ...
              r"\bottomrule", r"\end{tabular}", r"\par\smallskip\parbox{\linewidth}{\footnotesize Old endpoints are the last validated partial states before timeout, not 20-scan results. The 120-s bound covers the complete process, including imports, extraction, checkpoint validation, and output; it is not a completed coloring time. Sort process wall time is " + texstat(new["subprocess_s_complete_only"]) + r" s. Observed process peak working set is " + texstat(new["observed_peak_RSS_MiB"], 1) + r" MiB for completed Sort runs and " + texstat(old["observed_peak_RSS_MiB"], 1) + r" MiB up to censoring for Old; these memory endpoints represent different completed scan counts. Values are mean $\pm$ sample SD across three independent processes, including the screening execution reused as repetition 1.}", r"\end{table}", ""]
...
def record_postprocessor_version():
    snapshot = OUT / "source_snapshots/run_tifs_balance_repeats_20260910_measured.py"
    manifest = OUT / "source_manifest.json"
    if snapshot.exists() and manifest.exists():
        archived = OUT / "measurement_source_manifest.json"
        if not archived.exists():
            shutil.copyfile(manifest, archived)
        current = json.loads(archived.read_text(encoding="utf-8"))
        current[str(Path(__file__).relative_to(ROOT))] = hashlib.sha256(Path(__file__).read_bytes()).hexdigest()
        current[str(snapshot.relative_to(ROOT))] = hashlib.sha256(snapshot.read_bytes()).hexdigest()
        study.write_json(manifest, current)
        study.write_json(OUT / "source_version_provenance.json", dict(
            measurement_runner_snapshot=str(snapshot.relative_to(ROOT)),
            measurement_runner_sha256=hashlib.sha256(snapshot.read_bytes()).hexdigest(),
            current_postprocessor_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
            note="After measurements finished, only summary null handling and table wording/footnote output were corrected. The worker implementation and raw results were unchanged."))
