...
def plot_interval_coloring() -> None:
    ...
    rows = [
        (2.48, "Color 1", [(0, 2, r"$u_1 : [0,2]$"), (3, 4, r"$u_2 : [3,4]$"), (5, 6, r"$u_3 : [5,6]$")]),
        (1.48, "Color 2", [(0, 4, r"$u_4 : [0,4]$"), (5, 6, r"$u_5 : [5,6]$")]),
        (0.48, "Color 3", [(0, 6, r"$u_6 : [0,6]$")]),
    ]

    for color_index, (y, _label, intervals) in enumerate(rows, start=1):
        face, edge = colors[color_index]
        for left, right, text in intervals:
            ax.add_patch(
                Rectangle(
                    (left - 0.5, y - 0.23),
                    right - left + 1.0,
            ...
            ax.text((left + right) / 2, y, text, ha="center", va="center", fontsize=6.1)

    ax.axvline(3, ymin=0.07, ymax=0.94, color="#222222", linestyle=(0, (3, 2)), linewidth=0.85)
    ax.text(3.06, 3.12, r"target rank $r=3$", ha="left", va="bottom", fontsize=6.5)
    ax.set_yticks([row[0] for row in rows])
    ax.set_yticklabels([row[1] for row in rows], fontsize=6.1)
    ax.set_xticks(range(7))
    ax.set_xlabel("target rank", fontsize=6.4, labelpad=1.5)
    ax.grid(axis="x", color="#d7d7d7", linewidth=0.35)
    ...
    save(fig, "sparsetreepir_interval_coloring_figure")
