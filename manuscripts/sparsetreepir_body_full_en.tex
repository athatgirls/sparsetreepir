...
Table~\ref{tab:real_gain_sources} separates the sources of improvement. SparseTreePIR stores only non-default proof records, replaces the height-$h$ pruned TreePIR query pattern with $m$ subqueries, and balances per-color subdatabases. \ActiveBalance{} reaches the structural lower bound $B(T)$ on five of the six height-$128$ workloads, is within $1.045\times$ on the FuelLabs workload, and reduces the maximum subdatabase size by $10.8\%$--$21.6\%$ relative to a node-local greedy initializer.
