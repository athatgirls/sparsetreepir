...
\end{aligned}
\]
whose subdatabase sizes are $(2,4,4,2)$. The coloring is valid, but it is visibly uneven. By contrast, subtree recoloring can transform the same interval forest into
\[
\begin{aligned}
...
A valid coloring guarantees that each proof needs at most one real record from each color subdatabase, but it does not guarantee balanced subdatabase sizes. A highly imbalanced coloring can make one PIR query much more expensive than the others. \ActiveBalance{} is a load-balancing procedure that preserves the active width $m$ and changes only the assignment of records to colors. It evaluates subtree recolorings using the lexicographic load vector
\[
\Psi(\varphi)=
