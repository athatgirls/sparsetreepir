...
The following claims are stated relative to the record-based one-query-per-color model just defined. In particular, the retrieval-width result is not a lower bound for every conceivable private proof-retrieval protocol. It characterizes the color count and batch profile for the model where each PIR-retrievable sibling node is assigned to one color subdatabase and each retrieval sends one subquery per color.

\begin{theorem}[Exact retrieval width for one-query-per-color retrieval]
\label{thm:mincolor}
Let $n=|\mathcal{L}|\ge 2$ and let
\[
m=\max_{\ell\in\mathcal{L}} |\mathsf{Path}(\ell)|.
\]
For a fixed height-$h$ SMT snapshot, $m$ is the exact retrieval width of the one-query-per-color model: fewer than $m$ colors cannot support all target proofs, while $m$ colors are sufficient for a proper coloring of $\mathcal{A}(T)$. A feasible $m$-coloring can be computed in $O(N)$ time once the interval forest is available. Moreover,
\[
m\le \min\{h,n-1\},
\]
...
\begin{proof}
For the lower bound, choose an occupied leaf $\ell^\star$ whose PIR-retrievable sibling set has size $m$. All nodes in $\mathsf{Path}(\ell^\star)$ appear together in one proof, so any proper coloring must assign pairwise distinct colors to these $m$ nodes. Hence at least $m$ colors are necessary.

For the upper bound, recall that the target-rank intervals $\{I_{\mathsf{srv}}(u)\}$ form a laminar family, and two PIR-retrievable sibling nodes can co-occur in a proof exactly when their intervals intersect. In a laminar family, interval intersection is equivalent to ancestor-descendant relation in the interval forest. Therefore, assigning every node the color equal to its depth in that forest is proper: two nodes with the same depth cannot be in an ancestor-descendant relation, hence their intervals are disjoint and they never appear in the same proof. This depth coloring uses exactly the maximum forest depth, which is precisely $m$. The coloring is computed by a single depth-first traversal, hence in $O(N)$ time.

For the structural bound, every PIR-retrievable sibling node on the proof path of a target leaf $\ell$ corresponds to one ancestor where the sibling subtree is non-empty. These ancestors are at distinct SMT levels, so there are at most $h$ of them. Moreover, the non-empty sibling subtrees witnessed along one target path are pairwise disjoint, and each contains at least one occupied leaf different from $\ell$. Therefore, one target leaf can have at most $n-1$ PIR-retrievable sibling nodes, proving $m\le \min\{h,n-1\}$.

The same observation gives the compressed-skeleton interpretation: after contracting unary non-default paths, only branching ancestors contribute non-default proof records, and $m$ is the maximum number of such branching nodes on any compressed root-to-leaf path.

For tightness, let $k=\min\{h,n-1\}$ and choose the target leaf with logical bit string $0^h$. For each depth $d=0,\ldots,k-1$, add one occupied leaf in the sibling subtree of the target path at that depth, for example the bit string $0^d1\,0^{h-d-1}$. These $k$ leaves make all $k$ corresponding sibling subtrees non-empty, so the target has $k$ PIR-retrievable sibling nodes. If more than $k+1$ occupied leaves are required, add them arbitrarily without removing any of these witnesses. Hence the bound is attainable.
\end{proof}
...
\label{prop:unique}
If $\varphi$ is a proper coloring, then for every occupied leaf $\ell\in\mathcal{L}$ and every color $c\in[m]$, the set $\mathsf{Path}(\ell)$ contains at most one PIR-retrievable sibling node of color $c$.
\end{proposition}
...
\label{prop:batch}
Assume a correct single-item PIR backend. Then any proper coloring lets the client recover all PIR records of a target leaf $\ell$ using one batch-PIR request with exactly $m$ PIR subqueries.
\end{proposition}

\begin{proof}
By Proposition~\ref{prop:unique}, each color contributes at most one required record to $\mathsf{Path}(\ell)$. Hence the client retrieves at most one real item from each subdatabase, and uses a dummy query otherwise. The extracted real items are exactly the non-default proof material that must be supplied through PIR.
\end{proof}
...
\begin{proof}
Fix any efficient adversary and any two target leaves $\ell_0,\ell_1$ chosen in the game. Condition on the setup leakage $\mathcal{L}_{\mathsf{setup}}(T)$: the metadata tables, color assignment, subdatabase sizes, retrieval width $m$, and the fact that every retrieval sends exactly one query to every color subdatabase are all already known. The only remaining transcript components are the $m$ PIR queries themselves.

Consider one color $c$. For target $\ell_b$, the client either has a unique real target index in $\mathcal{D}_c$ or has no required PIR record of that color and therefore sends a dummy query. If both targets induce real queries, query privacy of the underlying PIR makes the two selected indices indistinguishable. If one target induces a real query and the other induces a dummy query, the dummy-query assumption makes the dummy transcript indistinguishable from a well-formed PIR query over the same subdatabase, and query privacy hides the selected index. If both induce dummy queries, the distributions are identical by construction.
...
\begin{proof}
Suppose two distinct nodes $u,v$ satisfy $\varphi(u)=\varphi(v)=c$ and $I_{\mathsf{srv}}(u)\cap I_{\mathsf{srv}}(v)\neq\emptyset$. Then any occupied leaf $\ell\in I_{\mathsf{srv}}(u)\cap I_{\mathsf{srv}}(v)$ has both $u$ and $v$ in its proof-specific PIR record set, contradicting proper coloring.
\end{proof}

\begin{theorem}[Structural lower bound for color-subdatabase balance]
\label{thm:structlb}
Let $N=|\mathcal{A}(T)|$ and let $m$ be the retrieval width induced by the record-based model. For a node $v$ in the interval forest, let $d(v)$ be the number of PIR-retrievable nodes on the forest path from its root to $v$, including $v$, and let $\operatorname{desc}(v)$ be the number of strict descendants of $v$ in the interval forest. Define
\[
B(T)=
\max\left\{
...
\]
Then $U/B(T)$ is an instance-wise balance certificate: if $\operatorname{OPT}$ is the smallest possible largest subdatabase among all proper colorings over $[m]$, then $U/\operatorname{OPT}\le U/B(T)$. Moreover, Algorithm~\ref{alg:activebalance} terminates at a subtree-permutation local optimum with respect to the lexicographic load vector $\Psi$.
\end{proposition}

\begin{proof}
Let $\operatorname{OPT}$ be the minimum possible value of $\max_c|\mathcal{D}_c|$ among all proper colorings over $[m]$. By Theorem~\ref{thm:structlb}, $\operatorname{OPT}\ge B(T)$. Therefore
\[
\frac{U}{\operatorname{OPT}}\le \frac{U}{B(T)}.
\]
This gives an a posteriori balance certificate for the returned instance.

For termination, observe that every accepted subtree permutation is required to strictly decrease the lexicographic load vector $\Psi$, while rejected moves do not change the coloring. There are only finitely many proper colorings of the interval forest over $[m]$. Thus the algorithm cannot accept infinitely many moves. When it stops because no improving move is found, no candidate subtree color permutation considered by Algorithm~\ref{alg:activebalance} can further decrease $\Psi$, which is precisely subtree-permutation local optimality under this move set.
\end{proof}

\begin{theorem}[Indexing complexity]
\label{thm:index}
Let $N=|\mathcal{A}(T)|$ be the total number of PIR-retrievable sibling nodes, and let $N_c=|\mathcal{D}_c|$ be the size of color subdatabase $c$. Then the interval-based preprocessing and query-indexing complexities satisfy
\begin{align*}
...
\begin{proof}
Computing the intervals of all PIR-retrievable nodes takes $O(N)$ time. Sorting the interval tables costs
\[
...
Fix one proper coloring of an interval forest with $N$ PIR-retrievable nodes and retrieval width $m$. Then one execution of Algorithm~\ref{alg:activebalance} with at most $R_1$ accepted-or-rejected passes runs in
\[
O(R_1 N m^3)
\]
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item the backend workload is determined by the subdatabase-size vector $\{|\mathcal{D}_c|\}_{c=1}^{m}$: a sequential execution performs one PIR query over each $\mathcal{D}_c$, while a parallel execution is bottlenecked by the largest per-color backend cost.
\end{enumerate}
\end{theorem}

\begin{proof}
The first item follows from Proposition~\ref{prop:batch} and Proposition~\ref{prop:complete}: each color returns at most one required non-default sibling digest, and default-empty levels are filled locally. The second item is exactly Theorem~\ref{prop:privacy}. The PIR database consists of one digest for each PIR-retrievable sibling node by construction, while the metadata tables store a constant number of fields per node, so their size is $O(N)$. The fourth item combines Theorem~\ref{thm:complete} with the interval-search bound in Theorem~\ref{thm:index}. Finally, the online protocol issues one independent PIR subquery to each color subdatabase. Therefore backend work composes over the sizes of the searched subdatabases, with the sequential and parallel interpretations stated above.
\end{proof}
