...
Figure~\ref{fig:workflow} summarizes how SparseTreePIR transforms ordinary SMT proof serving into private proof retrieval.
...
\toprule
Target rank & 0 & 1 & 2 & 3 & 4 & 5 & 6 \\
\midrule
Color 1 &
\multicolumn{3}{c}{\fbox{\(\;u_1:[0,2]\;\)}} &
\multicolumn{2}{c}{\fbox{\(\;u_2:[3,4]\;\)}} &
\multicolumn{2}{c}{\fbox{\(\;u_3:[5,6]\;\)}} \\
Color 2 &
\multicolumn{5}{c}{\fbox{\(\;u_4:[0,4]\;\)}} &
\multicolumn{2}{c}{\fbox{\(\;u_5:[5,6]\;\)}} \\
Color 3 &
\multicolumn{7}{c}{\fbox{\(\;u_6:[0,6]\;\)}} \\
\bottomrule
\end{tabular}
...
\State Refine $\varphi\gets\ActiveBalance{}(\mathcal{F},\varphi,R_1)$ by subtree recoloring
...
We use six SMT key sets: five account-key snapshots derived from public Polygon zkEVM and ZKsync Era RPC data, and one FuelLabs SMT test-vector workload. Each key set is mapped into height-$128$ and height-$256$ coordinate spaces, with duplicate coordinates merged. These are reproducible account-key snapshots rather than full archival state-tree dumps; the artifact records source identifiers, extraction scripts, command lines, and raw CSV outputs.

All backend measurements were run on Ubuntu 22.04.5 with Linux 6.8, a 13th Gen Intel Core i5-13500HX CPU with 20 hardware threads, 15 GiB RAM, Python 3.10.12, Go 1.18.1, GCC/G++ 11.4, and CMake 3.22. Implementation measurements use the height-$128$ workloads and repeat each backend experiment over three target-sampling seeds with $50$ occupied targets per workload. Online communication counts backend upload and download bytes for the measured PIR calls; preprocessing and timing columns refer only to the measured backend calls, not one-time layout construction or metadata publication.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{0.47\linewidth}r@{\quad}l}
\toprule
Comparison metric & Gain & Relative scale \\
\midrule
Pruned TreePIR subqueries & \(9.5\times\) & \rule{3.25cm}{4pt} \\
Pruned TreePIR max size & \(2.5\times\) & \rule{0.86cm}{4pt} \\
SimplePIR communication & \(2.1\times\) & \rule{0.72cm}{4pt} \\
PIANO communication & \(2.3\times\) & \rule{0.79cm}{4pt} \\
\bottomrule
\end{tabular}
...
This appendix gives proof details omitted from the main text. They are included for auditability; the main correctness, privacy, and complexity claims are stated in Section~\ref{sec:correctness}.
