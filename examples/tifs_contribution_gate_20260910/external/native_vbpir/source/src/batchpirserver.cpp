...
void BatchPIRServer::populate_raw_db()
{
    ...
    // Open the JSON file
    ifstream file(std::string(std::getenv("TREEPIR_INPUT_DIR")) + "/treedata/" + filename);
    if (!file.is_open()) {
...
void BatchPIRServer::read_colorBD() {
    ...
    for (uint64_t i = 0; i < tree_height_; i++) {
        ...
        std::string full_path = std::string(std::getenv("TREEPIR_INPUT_DIR")) + "/colorSubDB/" + filename;
