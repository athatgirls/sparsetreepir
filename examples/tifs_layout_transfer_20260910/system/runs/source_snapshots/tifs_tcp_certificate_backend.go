// Separate certificate-record audit prototype, 2026-09-10. This file is built
...
type QueryResult struct {
	...
	RouteMS            float64            `json:"record_hash_and_routing_ms"`
...
func runClient(address, inputPath, resultPath string) {
	...
	result := ClientResult{Schema: 1, Method: header.Method, PID: os.Getpid(), ServerAddress: address, BootstrapMS: ms(start), BootstrapUpload: conn.WrittenBytes, BootstrapDownload: conn.ReadBytes, MetadataBytes: len(metadata), SlotsBytes: len(slots), CachePayloadBytes: len(cache), Warmup: input.Warmup, Scope: "Separate client/server processes; one persistent TCP connection on loopback. End-to-end includes local record hashing/routing, query generation, serialization, transport, server answer, decode, and SMT root verification. Excludes initial bootstrap and process launch; bootstrap separately measured. Wire bytes count successful socket application reads/writes including framing, not TCP/IP headers or retransmissions."}
	...
	for sample := -input.Warmup; sample < len(input.Targets); sample++ {
		...
		row := QueryResult{Sample: sample, CTIndex: target.CTIndex, LogicalQueries: len(colors)}
		whole := time.Now()
		phase := time.Now()
		record, e := base64.StdEncoding.DecodeString(target.LeafInputBase64)
		must(e)
		value := sha256.Sum256(record)
		slot := value[:16]
